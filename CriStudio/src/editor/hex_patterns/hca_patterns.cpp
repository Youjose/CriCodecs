...
void add_hca_patterns(std::vector<HexPatternRange>& out, uint64_t total_size, std::span<const uint8_t> prefix) {
    ...
    add(out, 0, header_size == 0 ? 8 : header_size, QCoreApplication::translate("Editor.HcaPatterns", "HCA chunk header table"), tone(0), total_size);
    add_field(out, 0, 4, QCoreApplication::translate("Editor.HcaPatterns", "HCA magic"), ascii_value(prefix, 0, 4), 0, total_size);
    add_field(out, 4, 2, QCoreApplication::translate("Editor.HcaPatterns", "version"), hex_value(cricodecs::io::read_be<uint16_t>(prefix.data() + 4), 4), 1, total_size);
    add_field(out, 6, 2, QCoreApplication::translate("Editor.HcaPatterns", "header size"), QString::number(header_size), 2, total_size);
    ...
    while (cursor + 4 <= prefix.size() && cursor < header_size && chunk_color < 48) {
        ...
        if (chunk_id == 0x666D7400u && cursor + 0x10 <= prefix.size()) {
            add(out, cursor, 0x10, QCoreApplication::translate("Editor.HcaPatterns", "HCA fmt chunk"), tone(chunk_color++), total_size);
            ...
            add(out, cursor, 0x10, QCoreApplication::translate("Editor.HcaPatterns", "HCA comp chunk"), tone(chunk_color++), total_size);
            ...
            add(out, cursor, 0x0C, QCoreApplication::translate("Editor.HcaPatterns", "HCA dec chunk"), tone(chunk_color++), total_size);
            const auto frame_size = cricodecs::io::read_be<uint16_t>(prefix.data() + cursor + 4);
            ...
            add(out, cursor, 0x06, QCoreApplication::translate("Editor.HcaPatterns", "HCA ath chunk"), tone(chunk_color++), total_size);
            ...
            add(out, cursor, 0x06, QCoreApplication::translate("Editor.HcaPatterns", "HCA ciph chunk"), tone(chunk_color++), total_size);
            ...
            add(out, cursor, 0x10, QCoreApplication::translate("Editor.HcaPatterns", "HCA loop chunk"), tone(chunk_color++), total_size);
            ...
            add(out, cursor, 0x08, QCoreApplication::translate("Editor.HcaPatterns", "HCA vbr chunk"), tone(chunk_color++), total_size);
            ...
            add(out, cursor, 0x08, QCoreApplication::translate("Editor.HcaPatterns", "HCA rva chunk"), tone(chunk_color++), total_size);
    ...
    if (header_size < total_size) {
        add(out, header_size, total_size - header_size, QCoreApplication::translate("Editor.HcaPatterns", "HCA frames"), tone(2), total_size);
    }
}
...
void add_hca_repeats(std::vector<HexPatternRepeat>& out, uint64_t total_size, std::span<const uint8_t> prefix) {
    ...
    add_repeat(out, header_size, frame_size, frame_size, count, QCoreApplication::translate("Editor.HcaPatterns", "HCA frame"), tone(12), {
        HexPatternRepeatField{.offset = 0, .size = 2, .name = QCoreApplication::translate("Editor.HcaPatterns", "sync"), .color = tone(13), .value_kind = HexPatternValueKind::U16BE},
        HexPatternRepeatField{.offset = 2, .size = 2, .name = QCoreApplication::translate("Editor.HcaPatterns", "noise/evaluation bits"), .color = tone(14), .value_kind = HexPatternValueKind::U16BE},
        HexPatternRepeatField{.offset = 4, .size = frame_size > 6 ? static_cast<uint64_t>(frame_size - 6) : 0u, .name = QCoreApplication::translate("Editor.HcaPatterns", "packed scalefactors/spectra"), .color = tone(15), .value_kind = HexPatternValueKind::HexBytes},
        HexPatternRepeatField{.offset = frame_size > 1 ? static_cast<uint64_t>(frame_size - 2) : 0u, .size = 2, .name = QCoreApplication::translate("Editor.HcaPatterns", "frame checksum"), .color = tone(16), .value_kind = HexPatternValueKind::U16BE}
    }, total_size);
}
