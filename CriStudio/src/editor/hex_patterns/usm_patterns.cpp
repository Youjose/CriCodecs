...
void add_usm_chunk_patterns_from_header(
    ...
    const auto payload_offset = cricodecs::io::read_be<uint16_t>(header.data() + 0x08);
    ...
    add_field(out, base_offset + 0x08, 2, QStringLiteral("chunk.payload_offset"), QString::number(payload_offset), 2, total_size);
    ...
    add_field(out, base_offset + 0x0D, 1, QStringLiteral("chunk.reserved_0d"), hex_value(header[0x0D], 2), 6, total_size);
    add_field(out, base_offset + 0x0E, 2, QStringLiteral("chunk.payload_type_and_flags"), hex_value(cricodecs::io::read_be<uint16_t>(header.data() + 0x0E), 4), 7, total_size);
    ...
    add_field(out, base_offset + 0x18, 4, QStringLiteral("chunk.reserved_18"), hex_value(cricodecs::io::read_be<uint32_t>(header.data() + 0x18), 8), 11, total_size);
    add_field(out, base_offset + 0x1C, 4, QStringLiteral("chunk.reserved_1c"), hex_value(cricodecs::io::read_be<uint32_t>(header.data() + 0x1C), 8), 12, total_size);
    if (chunk_size > payload_offset) {
        const auto absolute_payload_offset = base_offset + 0x08u + payload_offset;
        add(out, absolute_payload_offset, chunk_size - payload_offset, QCoreApplication::translate("Editor.UsmPatterns", "USM chunk payload"), tone(2), total_size);
    }
}
...
void add_usm_chunk_patterns_at(std::vector<HexPatternRange>& out, uint64_t total_size, std::span<const uint8_t> prefix, uint64_t base_offset) {
    ...
    const auto header_offset = cricodecs::io::read_be<uint16_t>(prefix.data() + base + 0x08);
